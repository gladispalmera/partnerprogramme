          ...
          <div className="grid grid-cols-[50%_50%]">
            <div className="sticky left-0 z-10 bg-transparent">
              ...
              {featureData.map((feature, index) => (
                <div 
                  key={index} 
                  className={`p-4 pl-4 border-b border-gray-100 text-left text-[#243D48] font-medium h-[129px] flex items-center text-base sticky left-0 z-10 ${index % 2 === 0 ? 'bg-gray-50' : 'bg-transparent'} ${index === 0 ? 'rounded-tl-[12px]' : ''} ${index === featureData.length - 1 ? 'rounded-bl-[12px]' : ''}`}
                >
                  <div>{feature.name}</div>
                </div>
              ))}
              
              <div className="p-4 font-semibold text-[#243D48] text-left flex items-center sticky left-0 z-10 bg-transparent h-[60px] rounded-bl-[12px]">
              ...
              <div className="grid grid-cols-3 min-w-[600px]">
                ...
                {featureData.map((feature, index) => (
                  <React.Fragment key={index}>
                    <div className={`p-4 border-b border-[#243D48] text-center h-[129px] flex items-center justify-center text-base ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}>
            ...
            <div className="border-collapse relative max-w-[1200px] mx-auto">
              {featureData.map((feature, index) => (
                <div key={index} className={`grid grid-cols-[489.75px_1fr_1fr_1fr] ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}>
                  ...
                  <div className="p-4 border-b border-[#243D48] text-center min-h-[70px] flex items-center justify-center">
                    {feature.bronze ? <CheckIcon /> : <CrossIcon />}
                  </div>
                  <div className="p-4 border-b border-[#243D48] text-center min-h-[70px] flex items-center justify-center">
                    {feature.silver ? <CheckIcon /> : <CrossIcon />}
                  </div>
                  <div className={`p-4 border-b border-[#243D48] text-center min-h-[70px] flex items-center justify-center ${index === featureData.length - 1 ? '' : ''}`}>
                    {feature.gold ? <CheckIcon /> : <CrossIcon />}
                  </div>
                </div>
              ))}
